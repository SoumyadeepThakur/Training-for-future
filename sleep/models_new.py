...
class OrdinalClassifier(nn.Module):
    ...
    def __init__(self, data_shape, hidden_shape, time_shape, out_shape):

        super(OrdinalClassifier, self).__init__()
        self.model_0_0 = nn.Linear(data_shape, hidden_shape)
        self.model_0_1 = nn.Linear(data_shape, hidden_shape)
    
        self.model_1 = nn.Linear(2*hidden_shape, hidden_shape)
        self.model_2 = nn.Linear(hidden_shape, out_shape)
    ...
    def forward(self, X_1, X_2):
        ...
        hidden_1 = self.relu(self.model_0_0(X_1))
        hidden_2 = self.relu(self.model_0_1(X_2))

        hidden = torch.cat([hidden_1, hidden_2], dim=1)

        res = self.relu(self.model_2(self.relu(self.model_1(hidden))))
