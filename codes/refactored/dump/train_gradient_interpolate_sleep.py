...
class PredictionModel(nn.Module):
    ...
    def __init__(self, data_shape, hidden_shape, out_shape, time2vec=False):
        ...
        self.layer_2 = nn.Linear(hidden_shape, hidden_shape//2)
        nn.init.kaiming_normal_(self.layer_2.weight)
        nn.init.zeros_(self.layer_2.bias)
        self.relu_2 = TimeReLU(hidden_shape//2, self.time_dim, True)

        self.layer_3 = nn.Linear(hidden_shape//2, out_shape)
...
def train(X_data, Y_data, U_data, A_data, num_indices, source_indices, target_indices):
    ...
    classifier = PredictionModel(671, 400, 1, True)
...
if __name__ == "__main__":
    ...
    X_data, Y_data, A_data, U_data = load_sleep('shhs1-dataset-0.15.0.csv')
    preprocess_sleep(X_data, [0, 1])
