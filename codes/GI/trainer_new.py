...
class GradRegTrainer():
	...
	def train_classifier(self,past_dataset=None,encoder=None,inc_finetune=False):
		...
		if not self.inc_finetune:
			...
			#print(len(self.cumulative_data_indices[-1]))
			...
			for epoch in range(self.CLASSIFIER_EPOCHS):
				...
				#for params in self.classifier.layer_0.parameters():
				#	print(params)
				class_loss = 0
				num_batch = 0
				...
				print("Epoch %d Loss %f"%(epoch,class_loss/num_batch),flush=False)
				if self.schedule:
					self.scheduler.step()
	...
	def train(self):
		...
		print("Seed - {}".format(self.seed),file=self.log)
