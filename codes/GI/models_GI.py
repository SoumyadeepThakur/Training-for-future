...
class TimeReLU(nn.Module):
	...
	def __init__(self, data_shape, time_shape, leaky=False,use_time=True):
		...
		self.model_0 = nn.Linear(time_shape, 1)
		
		self.model_1 = nn.Linear(1, data_shape)

		self.time_dim = time_shape        

		if self.leaky:
			# self.model_alpha = nn.Linear(time_shape,data_shape)
			self.model_alpha_0 = nn.Linear(time_shape, 1)
			
			self.model_alpha_1 = nn.Linear(1, data_shape)
