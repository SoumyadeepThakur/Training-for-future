...
class CrossGradTrainer():
	...
	def train_classifier(self):
		...
		past_data = ClassificationDataSet(self.data_path, indices=self.cumulative_data_indices[-1],**self.dataset_kwargs)
		past_dataset = torch.utils.data.DataLoader((past_data),self.BATCH_SIZE,True)
		for epoch in range(self.CLASSIFIER_EPOCHS):
	...
	def train_cross_grad(self):
		...
		for idx in range(1, len(self.source_domain_indices)):
			...
			data_set = self.DataSet(self.datapath, source_indices=source_indices,target_indices=target_indices,**self.dataset_kwargs) #RotMNISTCGrad(source_indices,grad_target_indices,BIN_WIDTH,src_indices[0]-1,6,src_indices[idx]-1)
